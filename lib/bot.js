...
class Bot {
  ...
  handleMessage (message) {
    ...
    if (message.type === 'message' && message.text) {
      ...
      } else if (message.event === 'direct_mention' && message.text.startsWith('find ')) {
        const jql = message.text.slice(5);
        //this.bot.reply(message, this.jira.toString(), (err) => { logger.error(err) })
        this.jira.searchJira(jql).then(({issues}) => {
          issues.slice(0,10).forEach(issue => {
           response.attachments = [this.issueResponse(issue, 'short')];
           this.bot.reply(message, response, (err) => {
             if (err) {
               logger.error('Unable to respond', err);
             } else {
      ...
      } else if (message.text.match(/-?\d+[mhdw]/i)) {
        logger.info(message)
        const updated = ('-' + message.text).match(/-\d+[mhdw]/i);
        logger.info(`Show updates since ${updated}`);
        this.jira.searchJira(`assignee = currentUser() AND status in (Done, "In Review") AND updated > ${updated}`).then(({issues}) => {
